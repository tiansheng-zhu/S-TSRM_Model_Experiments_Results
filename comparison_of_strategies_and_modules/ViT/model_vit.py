...
class Thermohaline_Model(nn.Module):
    def __init__(self, in_shape, hid_S=16, hid_T=256, N_S=4,
        ...
        self.translator1 = nn.Sequential(
            ...
            # SwinSubBlock(dim=hid_T, input_resolution=(H_, W_), layer_i=1, mlp_ratio=mlp_ratio,drop=drop, drop_path=drop_path),
            # SwinSubBlock(dim=hid_T, input_resolution=(H_, W_), layer_i=0, mlp_ratio=mlp_ratio,drop=drop, drop_path=drop_path),
            ViTSubBlock(dim=hid_T, mlp_ratio=mlp_ratio, drop=drop, drop_path=drop_path),
            ViTSubBlock(dim=hid_T, mlp_ratio=mlp_ratio, drop=drop, drop_path=drop_path),
            ViTSubBlock(dim=hid_T, mlp_ratio=mlp_ratio, drop=drop, drop_path=drop_path),
