...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size, patch_size, in_chans, embed_dim):  # [160,360],2,1,128
        super(PatchEmbed, self).__init__()
        img_size = to_2tuple(img_size)  # (160,360)
        patch_size = to_2tuple(patch_size)  # (2,2)
        patches_resolution = [img_size[0] // patch_size[0], img_size[1] // patch_size[1]]  # [80,180]
        self.img_size = img_size  # (160,360)
        self.patch_size = patch_size  # (2,2)
        self.patches_resolution = patches_resolution  # [80,180]
        self.num_patches = patches_resolution[0] * patches_resolution[1]  # 80*180=14400
        self.in_chans = in_chans  # 1
        self.embed_dim = embed_dim  # 128
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)  # 1,128,(2,2),(2,2)
        self.norm = nn.LayerNorm(embed_dim)

    def forward(self, x):
        B, C, H, W = x.shape  # 1,1,160,360
        assert H == self.img_size[0] and W == self.img_size[1], \
            f"Input image size ({H}*{W}) doesn't match model ({self.img_size[0]}*{self.img_size[1]})."
        x = self.proj(x).flatten(2).transpose(1, 2)  # 1,80*180=14400,1
        x = self.norm(x)  # 1,80*180=14400,1
        return x
...
class PatchInflated(nn.Module):
    ...
    def __init__(self, in_chans, embed_dim, input_resolution, stride=2, padding=1, output_padding=1):
        super(PatchInflated, self).__init__()  # 1,128,[80,180],2,1,1

        stride = to_2tuple(stride)  # (2,2)
        padding = to_2tuple(padding)  # (1,1)
        output_padding = to_2tuple(output_padding)  # (1,1)
        self.input_resolution = input_resolution  # (80,180)
...
class SwinTransformerBlocks(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop=0., attn_drop=0., drop_path=0., norm_layer=nn.LayerNorm): # 128,(80,180),12,4,4,T,None,0,0,0.1,LayerNorm
        ...
        self.layers = nn.ModuleList([
            ...
            for i in range(depth)])  # 128,(80,180),4,0/2,4,T,None,0,0,0.1,LayerNorm
...
class SwinLSTMCell(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size, depth,
                 ...
                 drop_path=0., norm_layer=nn.LayerNorm):  # 128,(80,180),12,4,4,T,None,0,0,0.1,LayerNorm
        ...
        self.Swin = SwinTransformerBlocks(dim=dim, input_resolution=input_resolution, depth=depth,
                                          ...
                                          drop_path=drop_path, norm_layer=norm_layer) # 128,(80,180),12,4,4,T,None,0,0,0.1,LayerNorm

    def forward(self, xt, hidden_states):  # 1,80*180,1,None
        if hidden_states is None:
            B, L, C = xt.shape  # 1,80*180,1
            hx = torch.zeros(B, L, C).to(xt.device)  # 1,80*180,1
            cx = torch.zeros(B, L, C).to(xt.device)  # 1,80*180,1
...
class STconvert(nn.Module):
    ...
    def __init__(self, img_size, patch_size, in_chans, embed_dim, depths, num_heads, window_size,
                 ...
                 norm_layer=nn.LayerNorm):  # (160,360) ,2,1,128,[12],[4,8],4,4,T,None,0,0,0.1
        ...
        self.patch_embed = PatchEmbed(img_size=img_size, patch_size=patch_size,
                                      in_chans=in_chans, embed_dim=embed_dim)  # (160,360),2,1,128
        patches_resolution = self.patch_embed.patches_resolution  # [80,180]
        ...
        for i_layer in range(self.num_layers):
            layer = SwinLSTMCell(dim=embed_dim,
                                 ...
                                 norm_layer=norm_layer)  # 128,(80,180),12,4,4,T,None,0,0,0.1,LayerNorm

            self.layers.append(layer)

    def forward(self, x, h):  # 1,1,160,360

        x = self.patch_embed(x)  # 1,80*180,1
